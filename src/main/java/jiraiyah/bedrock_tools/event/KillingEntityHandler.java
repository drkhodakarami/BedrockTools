...
public class KillingEntityHandler implements ServerEntityCombatEvents.AfterKilledOtherEntity
{
    ...
    public void afterKilledOtherEntity(ServerWorld world, Entity entity, LivingEntity killedEntity)
    {
        ...
        if(killedEntity instanceof EndermanEntity &&
           entity.isPlayer())
        {
            if(((PlayerEntity) entity).getStackInHand(Hand.MAIN_HAND).isOf(ModItems.OBSIDIAN_SWORD))
            {
            ...
            if(((PlayerEntity) entity).getStackInHand(Hand.MAIN_HAND).isOf(ModItems.BEDROCK_SWORD))
            {
                world.spawnEntity(new ItemEntity(world,
                                                 killedEntity.getPos().x,
                                                 killedEntity.getPos().y,
